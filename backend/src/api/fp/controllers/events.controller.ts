...
} from "@fp/services/events.service";
import { createEvent as serviceCreateEvent } from "@fp/services/booking.service";
import {
  ...
  type DeleteEventDto,
} from "@fp/validators/events.validator";
import { ValidationError } from "@/lib/errors/app-errors";
import type { ExportFormat, FormattedCalendarEvent } from "@fp/types/events.types";
...
export const createEvent = async (c: Context) => {
  ...
  if (!validationResult.success) {
    console.error("EventsController - createEvent - validation error:", formatValidationError(validationResult.error));
    throw new ValidationError("Nieprawidłowe dane wejściowe");
  }
...
export const deleteEvent = async (c: Context) => {
  ...
  if (!validationResult.success) {
    console.error("EventsController - deleteEvent - validation error:", formatValidationError(validationResult.error));
    throw new ValidationError("Nieprawidłowe dane wejściowe");
  }
